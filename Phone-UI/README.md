...
Framework7 Core 9.1.2 and Framework7 Icons 5.0.5 are vendored under `vendor/`,
so production boot does not depend on a CDN or network connection. There is no
build step or wrapper.
...
ready. Asset failures show a retry control. Boot now hands off to the animated
multilingual Hello screen; swipe up (or activate its home indicator) to open
Home. Swipe sideways to move between three home pages. Tap an
...
Pull from the left side of the status bar to reveal the lock-style clock and
Notification Center, then swipe it upward to dismiss. Tap Search or pull down
below the status bar to reveal Spotlight; the top-right status corner stays
inert because Control Center is out of scope. Its input is visual only. Removal
changes the demo catalog for the current session; reload restores it. The
Notification Center flashlight toggles its button state only.
...
- `ios-boot-core.js`: multilingual greetings and the shared cancellable spring.
...
decorative except the rebuilt 60-mark analog clock, which uses system time.
...
Browser QA additionally exercises Hello-to-Home, the top-left clock pull,
paging, app opening/closing, editing/removal, Spotlight, and switching across
the 700px frame breakpoint.
